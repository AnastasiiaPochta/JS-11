...
const table = (array, key, boolean = true) => {
  const car = sort(array, key, boolean);
  const columns = [];
...
const calcResult = () => {
    ...
    divisionResult.innerHTML = `<strong>${result}</strong><br/>`;
}
...
const calc = () => {
    let fullPrice = prompt('Введіть ціну з ПДВ'); //Ввід ціни з ПДВ
    fullPrice = fullPrice.replace(`,`,`.`); //Заміна , на . для вірних розрахунків
    const pdv = 0.2; //Актуальна ставка ПДВ
    const result = (fullPrice/(1+pdv)).toFixed(2); //Ціна без ПДВ
    return alert (`Ціна без ПДВ дорвінює ${result}`);
}
...
const calc = () => {
    const pdv = 0.2;
    const resultValue = (fullPrice.value/(1+pdv)).toFixed(2);
    result.innerHTML = `<strong>${resultValue}</strong><br/>`;
}
fullPrice.oninput = calc;
